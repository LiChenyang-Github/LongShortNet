...
class IMG_ONE_FRAME_THREE_ARGOVERSEDataset(Dataset):
    """
    COCO dataset class.
    """
    ...
    def load_anno_from_ids(self, id_):
        ...
        # the negative 2(t-2) frame
        ## front fid
        if self.coco.dataset['images'][int(id_)]['fid'] == 0:
            im_ann_support_neg_2 = im_ann
        ...
        # the second frame of each video
        elif self.coco.dataset['images'][int(id_)]['fid'] == 1:
            im_ann_support_neg_2 = self.coco.loadImgs(id_ - 1)[0]

        else:
            im_ann_support_neg_2 = self.coco.loadImgs(id_ - 2)[0]

        im_name_support_neg_2 = im_ann_support_neg_2['name']
        im_sid_support_neg_2 = im_ann_support_neg_2['sid']
        ...
        support_neg_2_file_name = os.path.join(self.data_dir, 'Argoverse-1.1', 'tracking', self.seq_dirs[im_sid_support_neg_2], im_name_support_neg_2)
        ...
        return (res, support_res, img_info, resized_info, file_name, support_file_name, support_neg_2_file_name)
    ...
    def load_support_neg_2_resized_img(self, index):
        img = self.load_support_neg_2_image(index)
        r = min(self.img_size[0] / img.shape[0], self.img_size[1] / img.shape[1])
        resized_img = cv2.resize(
            img,
            (int(img.shape[1] * r), int(img.shape[0] * r)),
    ...
    def load_support_neg_2_image(self, index):
    ...
    def pull_item(self, index):
        ...
        res, support_res, img_info, resized_info, _, _, _ = self.annotations[index]

        img = self.load_resized_img(index)
        support_img = self.load_support_resized_img(index)
        support_neg_2_img = self.load_support_neg_2_resized_img(index)

        return img, support_img, support_neg_2_img, res.copy(), support_res.copy(), img_info, np.array([id_])
    ...
    def __getitem__(self, index):
        ...
        """
        img, support_img, support_neg_2_img, target, support_target, img_info, img_id = self.pull_item(index)

        if self.preproc is not None:

            img, support_img, support_neg_2_img, target, support_target = self.preproc((img, support_img, support_neg_2_img), (target, support_target), self.input_dim)

        return np.concatenate((img, support_img, support_neg_2_img), axis=0), (target, support_target), img_info, img_id
