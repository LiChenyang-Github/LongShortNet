...
class IMG_ANNO_ONE_TWO_THREE_ARGOVERSEDataset(Dataset):
    """
    COCO dataset class. 
    """
    def __init__(self, data_dir='/data/Datasets/', json_file='train.json',
        ...
        self.annotations_threex = self._load_coco_annotations_threex()
        ...
        self.speed_to_annos = {
            ...
            SpeedType.THREEX.value: self.annotations_threex,
        }
    ...
    def _load_coco_annotations_threex(self):
        return [self.load_anno_from_ids_threex(_ids) for _ids in self.ids]
    ...
    def load_anno_from_ids_threex(self, id_):
        im_ann = self.coco.loadImgs(id_)[0]
        width = im_ann["width"]
        height = im_ann["height"]
        im_name = im_ann['name']
        ...
        # the third to last frame of the last video
        elif int(id_) == seq_len-3:
            im_ann_support = self.coco.loadImgs(id_ - 2)[0]
        ...
        # the third to last frame of videos except the last video
        elif self.coco.dataset['images'][int(id_ + 3)]['fid'] == 0:
            im_ann_support = self.coco.loadImgs(id_ - 2)[0]

        # other cases
        else:
            im_ann_support = self.coco.loadImgs(id_ - 3)[0]
        ...
        # deal with annotations info of the case using still annotations
        ## back seq fid
        # the last four frames of the last video
        if id_ in [seq_len-1, seq_len-2, seq_len-3, seq_len-4]:
            anno_ids = self.coco.getAnnIds(imgIds=[int(seq_len)], iscrowd=False)
        ## back fid
        else:
            if self.coco.dataset['images'][int(id_)]['fid'] == 0:
        ...
        ## front fid
            ...
            # the third frame of videos except the last video
            elif self.coco.dataset['images'][int(id_)]['fid'] == 2:
                anno_ids = self.coco.getAnnIds(imgIds=[int(id_ + 2)], iscrowd=False)
