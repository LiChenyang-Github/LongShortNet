...
class DFPPAFPNFRAMEDIFF(nn.Module):
    """
    ...
    def __init__(
        ...
        depthwise=False,
        act="silu",
        frame_diff_type="feature", # "feature" or "input"
    ):
        super().__init__()
        self.backbone = CSPDarknet(depth, width, depthwise=depthwise, act=act)
        self.in_features = in_features
        self.in_channels = in_channels
        self.frame_diff_type = frame_diff_type
    ...
    def off_forward(self, input):
        ...
        if self.frame_diff_type == "feature":
            support_out_features = self.backbone(torch.split(input, 3, dim=1)[1])
        elif self.frame_diff_type == "input":
            support_out_features = self.backbone(torch.split(input, 3, dim=1)[0] - torch.split(input, 3, dim=1)[1])
        else:
            raise Exception(f'frame_diff_type must be in ["feature", "input"]')
        ...
        # 0.5 channel
        if self.frame_diff_type == "feature":
            pan_out2 = torch.cat([self.jian2(rurrent_pan_out2), self.jian2(rurrent_pan_out2 - support_pan_out2)], dim=1) + rurrent_pan_out2
            pan_out1 = torch.cat([self.jian1(rurrent_pan_out1), self.jian1(rurrent_pan_out1 - support_pan_out1)], dim=1) + rurrent_pan_out1
            pan_out0 = torch.cat([self.jian0(rurrent_pan_out0), self.jian0(rurrent_pan_out0 - support_pan_out0)], dim=1) + rurrent_pan_out0
        elif self.frame_diff_type == "input":
