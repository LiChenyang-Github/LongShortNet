...
class DFPPAFPNTHREEFRAME(nn.Module):
    """
    ...
    def __init__(
        ...
        if merge_mode == "concat":
            ...
            self.jian2_dynamic = Conv(
                        in_channels=int(in_channels[0] * width),
                        out_channels=int(in_channels[0] * width) // 4,
                        ksize=1,
                        stride=1,
                        act=act,
                    )
            ...
            self.jian1_dynamic = Conv(
                in_channels=int(in_channels[1] * width),
                out_channels=int(in_channels[1] * width) // 4,
                        ksize=1,
                        stride=1,
                        act=act,
                    )
            ...
            self.jian0_dynamic = Conv(
                in_channels=int(in_channels[2] * width),
                out_channels=int(in_channels[2] * width) // 4,
                        ksize=1,
                        stride=1,
                        act=act,
                    )
            ...
            raise Exception('The merge_mode must be in ["concat", "add"].')
    ...
    def off_forward(self, input):
        ...
        support_neg_2_out_features = self.backbone(torch.split(input, 3, dim=1)[2])
        support_neg_2_features = [support_neg_2_out_features[f] for f in self.in_features]
        [support_neg_2_x2, support_neg_2_x1, support_neg_2_x0] = support_neg_2_features

        support_neg_2_fpn_out0 = self.lateral_conv0(support_neg_2_x0)  # 1024->512/32
        support_neg_2_f_out0 = F.interpolate(support_neg_2_fpn_out0, size=support_neg_2_x1.shape[2:4], mode='nearest')  # 512/16
        support_neg_2_f_out0 = torch.cat([support_neg_2_f_out0, support_neg_2_x1], 1)  # 512->1024/16
        support_neg_2_f_out0 = self.C3_p4(support_neg_2_f_out0)  # 1024->512/16

        support_neg_2_fpn_out1 = self.reduce_conv1(support_neg_2_f_out0)  # 512->256/16
        support_neg_2_f_out1 = F.interpolate(support_neg_2_fpn_out1, size=support_neg_2_x2.shape[2:4], mode='nearest')  # 256/8
        support_neg_2_f_out1 = torch.cat([support_neg_2_f_out1, support_neg_2_x2], 1)  # 256->512/8
        support_neg_2_pan_out2 = self.C3_p3(support_neg_2_f_out1)  # 512->256/8

        support_neg_2_p_out1 = self.bu_conv2(support_neg_2_pan_out2)  # 256->256/16
        support_neg_2_p_out1 = torch.cat([support_neg_2_p_out1, support_neg_2_fpn_out1], 1)  # 256->512/16
        support_neg_2_pan_out1 = self.C3_n3(support_neg_2_p_out1)  # 512->512/16

        support_neg_2_p_out0 = self.bu_conv1(support_neg_2_pan_out1)  # 512->512/32
        support_neg_2_p_out0 = torch.cat([support_neg_2_p_out0, support_neg_2_fpn_out0], 1)  # 512->1024/32
        support_neg_2_pan_out0 = self.C3_n4(support_neg_2_p_out0)  # 1024->1024/32


        if self.merge_mode == "concat":
            pan_out2 = torch.cat([self.jian2_static(rurrent_pan_out2), self.jian2_dynamic(support_pan_out2),  self.jian2_dynamic(support_neg_2_pan_out2)], dim=1) + rurrent_pan_out2
            pan_out1 = torch.cat([self.jian1_static(rurrent_pan_out1), self.jian1_dynamic(support_pan_out1),  self.jian1_dynamic(support_neg_2_pan_out1)], dim=1) + rurrent_pan_out1
            pan_out0 = torch.cat([self.jian0_static(rurrent_pan_out0), self.jian0_dynamic(support_pan_out0),  self.jian0_dynamic(support_neg_2_pan_out0)], dim=1) + rurrent_pan_out0
        elif self.merge_mode == "add":
            # 0.5 channel
            pan_out2 = torch.cat([self.jian2(rurrent_pan_out2), self.jian2(support_pan_out2) + self.jian2(support_neg_2_pan_out2)], dim=1) + rurrent_pan_out2
            pan_out1 = torch.cat([self.jian1(rurrent_pan_out1), self.jian1(support_pan_out1) + self.jian1(support_neg_2_pan_out1)], dim=1) + rurrent_pan_out1
            pan_out0 = torch.cat([self.jian0(rurrent_pan_out0), self.jian0(support_pan_out0) + self.jian0(support_neg_2_pan_out0)], dim=1) + rurrent_pan_out0
    ...
    def forward(self, input, buffer=None, mode='off_pipe'):
        ...
        if mode=='off_pipe':
            ...
            # offline train mode
                ...
                raise Exception("The input must be 1/3 images(3/9 channels).")
            # three frames
            elif input.size()[1] == 9:
                output = self.off_forward(input)
